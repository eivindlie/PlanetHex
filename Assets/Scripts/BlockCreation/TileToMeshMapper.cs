...
namespace Meshes
{
    public static class TileToMeshMapper
    {
        ...
        var pos = tile.Center.ProjectToRadius(bottomRadius).AsVector();
        var vertexIndex = 0;
        for (var i = 0; i < tile.Boundary.Count; i++)
        {
            vertices[vertexIndex] = tile.Boundary[i].ProjectToRadius(bottomRadius).AsVector() - pos;
            vertices[offset + vertexIndex] = tile.Boundary[i].ProjectToRadius(bottomRadius + height).AsVector() - pos;
            var next = 1;
            if (i != 1 && i != 2)
            {
                next++;
                vertices[vertexIndex + 1] = tile.Boundary[i].ProjectToRadius(bottomRadius).AsVector() - pos;
                vertices[offset + vertexIndex + 1] = tile.Boundary[i].ProjectToRadius(bottomRadius + height).AsVector() - pos;
            }
            if (i == 0)
            {
                next++;
                vertices[vertexIndex + 2] = tile.Boundary[i].ProjectToRadius(bottomRadius).AsVector() - pos;
                vertices[offset + vertexIndex + 2] = tile.Boundary[i].ProjectToRadius(bottomRadius + height).AsVector() - pos;
            }
            if (i == count - 1)
            {
                next += 2;
